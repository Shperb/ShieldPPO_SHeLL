...
class KinematicObservation(ObservationType):
    ...
    def space(self) -> spaces.Space:
        return spaces.Tuple([spaces.Box(shape=(self.vehicles_count, len(self.features)), low=-np.inf, high=np.inf, dtype=np.float32), spaces.Discrete(5)])
    ...
    def observe(self) -> np.ndarray:
        ...
        # Reorder
        df = df[self.features]
        objects = self.env.road.objects
        if len(objects) > 0:	
            object_obs = [1,objects[0].position[0]-self.observer_vehicle.position[0], objects[0].position[1]-self.observer_vehicle.position[1]]	
        else:	
            object_obs = [0,0,0]	
        # df = df.append(pd.DataFrame(self.env.road., ignore_index=True)
        _from, _to, _id = self.observer_vehicle.lane_index
        is_left = _id > 0
        is_right = _id < len(self.env.road.network.graph[_from][_to]) - 1
        obs = df.values.copy()	
        obs = np.append(obs,object_obs)
        obs = np.append(obs,[is_left,is_right])
