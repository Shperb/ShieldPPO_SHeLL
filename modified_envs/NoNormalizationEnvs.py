...
from original_envs.cartpole import CartPoleEnv
...
class CartPoleWithCost(CartPoleEnv):
    ...
    def reset(self, param_ranges, seed = None, gan_output=None):
        # set random_seed as env seed
        super().reset(seed = seed)
        if gan_output:
            self.update_params(gan_output)
        else:
            self.__init__(param_ranges = param_ranges)
    ...
    def _reward(self, action: int) -> float:
        ...
        for vehicle in self.road.vehicles:
            if vehicle.lane_index == ("b", "c", 2) and isinstance(vehicle, ControlledVehicle):
                speed_reward = 0 if vehicle.target_speed == 0 else (
