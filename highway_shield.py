...
def train(args, env_name):
    ...
    base_path = f"./models/{folder_name}/{obs_name}_{curr_time}_{env_name}"
    print(base_path)
    ...
    while time_step <= max_training_timesteps:
        ...
        for t in range(1, max_ep_len + 1):
            ...
            if len(trajectory) > args.record_trajectory_length:
                trajectory.pop()
            if t > 1 and time_step >= args.pos_to_neg_threshold and no_safe_action and last_added_to_buffer == 1:
...
def get_args():
    ...
    parser.add_argument("--log_freq", type=int, default=100000,
