...
def placeholder_from_space(space):
    ...
    if isinstance(space, Box):
        ...
        #return tf.placeholder(dtype=tf.int32, shape=combined_shape(None, space.n))
    else: #elif isinstance(Tuple):
        return placeholder(dim=(81,))
...
def mlp_categorical_policy(x, a, mask, hidden_sizes, activation, output_activation, action_space):
    act_dim = action_space.n
    # valid_mask = np.zeros(act_dim)
    ...
    logits = mask*logits + (1-mask)* -1000000.0
...
Actor-Critics
"""
def mlp_actor_critic(x, a, mask, hidden_sizes=(64,64), activation=tf.tanh,
    ...
    with tf.variable_scope('pi'):
        policy_outs = policy(x, a, mask, hidden_sizes, activation, output_activation, action_space)
