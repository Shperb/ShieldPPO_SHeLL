...
def train(arguments = None):
    # max_training_timestamps - the maximal number of interactions of the agent with the environment (total actions)
    ...
    parser.add_argument("--max_training_timesteps", type=int, default=int(100000),
    ...
    parser.add_argument("--record_trajectory_length", type=int, default=1000,
                        help="Record trajectory length")
    parser.add_argument("--cpu", type=int, default=4,
                        help="Number of cpus")
    ## NEW ARGUMENTS
    ## TODO - the masking treshold can be set according to improvement in shield loss
    parser.add_argument("--masking_threshold", type=int, default=0,
                        help="Time step at which to start using the shield")
    parser.add_argument("--k_last_states", type=int, default=1,
                        help="K last states to update the policies based on")
    ...
    if agent == "PPO":
        ...
        ppo_agent = ShieldPPO(multi_task_env.state_dim, action_dim, lr_actor, lr_critic, gamma, K_epochs, eps_clip,
                              has_continuous_action_space, action_std, masking_threshold = masking_threshold, k_last_states = k_last_states)
    ...
    while time_step <= max_training_timesteps:
        ...
        # INITALIZE THE REWARDS&COSTS (CULUMATIVE)
        current_ep_reward = 0
        current_ep_cost = 0
        is_mistake = False
        if args.record_mistakes:
        ...
        for t in range(1, max_ep_len + 1):
            ...
            next_action = action[-1].item()
            state, reward, done, info = multi_task_env.step(next_action)
            last_states.append(state)
            ## make sure that there is always only k_last_states
            if len(last_states) > k_last_states:
                last_states = last_states[1:]
            trajectory.appendleft((next_action,state))
            ...
            if agent == "ShieldPPO" or agent == "RuleBasedShieldPPO":
                padded_states = prev_states.copy() # Create a copy of last_states to avoid modifying it
                while len(padded_states) < ppo_agent.k_last_states:
                    ## TODO (?) - VALUES FOR DUMMY STATES
                    ## THE INPUT OF THE STATES IS PADDED WHEN INSERTING IT TO THE SHIELD.
                    dummy_state = np.zeros_like(padded_states[0])
                    padded_states.insert(0, dummy_state)
                    ...
                    if args.record_mistakes:
                        ...
                        with open(trajectory_path, 'w') as f:
                            for item in reversed(trajectory):
                                f.write(f"{item}\n")
                    padded_states_tensor = torch.FloatTensor(padded_states) # Convert to tensor
                    ppo_agent.add_to_shield(padded_states_tensor, action, 0)
                else:
                    padded_states_tensor = torch.FloatTensor(padded_states)  # Convert to tensor
                    ppo_agent.add_to_shield(padded_states_tensor, action, 1)
