...
def run_polopt_agent(env_fn, 
    ...
    base_path = f"./models/{logger_kwargs['exp_name']}_{seed}_{curr_time}"
    ...
    m_ph = placeholder(env.action_space_size())
    ...
    ac_outs = actor_critic(x_ph, a_ph, m_ph, **ac_kwargs)
    pi, logp, logp_pi, pi_info, pi_info_phs, d_kl, ent, v, vc = ac_outs

    # Organize placeholders for zipping with data from buffer on updates
    buf_phs = [x_ph, a_ph, m_ph, adv_ph, cadv_ph, ret_ph, cret_ph, logp_old_ph]
    ...
    obs_shape = (81,)
    #act_shape = env.action_space_size()
    ...
    for epoch in range(epochs):
        ...
        for t in range(local_steps_per_epoch):
            ...
            #get mask
            valid_mask = np.full((env.action_space_size(),), 0.0)
            valid_actions = env.get_valid_actions()
            for a in valid_actions:
                valid_mask[a] = 1.0
            # Get outputs from policy
            #valid_mask = np.reshape(valid_mask, (1, env.action_space_size()))
            get_action_outs = sess.run(get_action_ops, 
                                       feed_dict={x_ph: o[np.newaxis], m_ph: valid_mask[np.newaxis]})
            ...
            env_name = env.get_current_env_name()
            ...
            if agent.reward_penalized:
                ...
                buf.store(o, a, valid_mask, r_total, v_t, 0, 0, logp_t, pi_info_t)
            else:
                buf.store(o, a, valid_mask, r, v_t, c, vc_t, logp_t, pi_info_t)
