...
class ShieldBuffer:
    ...
    def add(self, state_action):
        ...
        self.states_actions.append(state_action)
    ...
    def sample(self, n):
        ...
        if n > len(self.states_actions[:-1]):
            n = len(self.states_actions[:-1])
        states_action_to_sample = self.states_actions[:-1]
        epochs_batch = random.sample(states_action_to_sample, n)
        return epochs_batch
...
class ShieldPPO(PPO):
    def __init__(self, state_dim, action_dim, lr_actor, lr_critic, gamma, eps_clip, k_epochs_ppo, k_epochs_shield,
                 ...
                 masking_threshold, unsafe_tresh, use_gen_v2, shield_buffer_size, param_ranges=None):
        ...
        self.shield_buffer = PER_Buffer(shield_buffer_size)
    ...
    def add_to_shield(self, error, state_action):
        ...
        # self.shield_buffer.add(state_action)
        self.shield_buffer.add(error, state_action)
    ...
    def update_shield(self, batch_size):
        if self.shield_buffer.get_buffer_len() == 0:
            return 0
        if self.shield_buffer.get_buffer_len() <= batch_size:
            batch_size = self.shield_buffer.get_buffer_len()
        """
        batch: batch of prioritizied samples
        indices: the indices of the samples
        is_weight: the importance-sampling weights of the samples.
        """
        batch_samples, idxs, weights = self.shield_buffer.sample(batch_size)
        states_ = [step[0] for step in batch_samples]
        actions_ = [step[1] for step in batch_samples]
        costs_ = [step[2] for step in batch_samples]
        ...
        costs = torch.squeeze(torch.stack(costs_, dim=0)).detach().to(device)
        total_loss = 0.

        for _ in range(self.k_epochs_shield):
            self.shield_opt.zero_grad()
            loss = self.shield.loss(states, actions, costs)
            loss.backward()
    ...
    """
 # old one - before changing the shield buffer to ExperienceReplayBuffer

    def update_shield(self, shield_episodes_batch_size):
        if self.shield_buffer.get_buffer_len() == 0:
            return 0
        # -1 because we can't sample the last episode
        if self.shield_buffer.get_buffer_len() <= shield_episodes_batch_size:
